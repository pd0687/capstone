...
For each of the five time series, we will fit an AR(1,0,0) model, training over the past seven quarters to test the eighth quarter.  We will compare the actual level to the predicted one and compute the RMSE and MAPE as the window rolls through the time series.  We will vary the training window to see if shorter or longer windows provide better predictive power.
...
The deliverables include a summary of the resulting RMSE and MAPE for all five of the index levels, over each of the time series models.  Additionally, time series plots for each iteration can include the fitted model along with 1-, 2-, and 3-standard deviation confidence bands around the model.

The choice of final model might involve a tradeoff among:

* which model performs best in terms of RMSE or MAPE,
* which least frequently escapes the confidence bands, and
